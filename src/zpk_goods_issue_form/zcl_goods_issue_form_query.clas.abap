    ...
    TRY.
        ...
        IF lo_filter IS BOUND.
          ...
          LOOP AT lt_filters INTO DATA(ls_filter).
            IF ls_filter-name = 'HEADOFDEPARTMENT'.
              DATA(lr_tp) = lt_filters[ name = 'HEADOFDEPARTMENT' ]-range.
              READ TABLE lr_tp INTO DATA(ls_range) INDEX 1.
              DATA(lv_tp) = ls_range-low.
            ELSEIF ls_filter-name = 'CASHIER'.
              DATA(lr_tkho) = lt_filters[ name = 'CASHIER' ]-range.
              READ TABLE lr_tkho INTO ls_range INDEX 1.
              DATA(lv_tkho) = ls_range-low.
            ELSEIF ls_filter-name = 'DIRECTOR'.
              DATA(lr_ktoan) = lt_filters[ name = 'DIRECTOR' ]-range.
              READ TABLE lr_ktoan INTO ls_range INDEX 1.
              DATA(lv_ktoan) = ls_range-low.
            ELSEIF ls_filter-name = 'DEPARTMENT'.
              DATA(lr_depart) = lt_filters[ name = 'DEPARTMENT' ]-range.
              READ TABLE lr_depart INTO ls_range INDEX 1.
              DATA(lv_bp) = ls_range-low.
            ENDIF.
          ENDLOOP.
        ENDIF.
...
*             SalesOrderNumber     = |{ ls_item-SpecialStockIdfgSalesOrder }/{ ls_item-SpecialStockIdfgSalesOrderItem }|
             CustomerName         = ls_item-customername
             documentheadertext   = ls_item-materialdocumentheadertext
             postingdate          = ls_item-postingdate  ).
        ENDIF.
        IF ls_item-SpecialStockIdfgSalesOrder IS NOT INITIAL AND ls_item-SpecialStockIdfgSalesOrderItem IS NOT INITIAL.
          lv_so = ls_item-SpecialStockIdfgSalesOrder.
          lv_item = ls_item-SpecialStockIdfgSalesOrderItem.
          SHIFT lv_so LEFT DELETING LEADING '0'.
          SHIFT lv_item LEFT DELETING LEADING '0'.
          ls_result-SalesOrderNumber = |{ lv_so }/{ lv_item }|.
        ELSEIF ls_item-SpecialStockIdfgSalesOrder IS NOT INITIAL.
          lv_so = ls_item-SpecialStockIdfgSalesOrder.
          SHIFT lv_so LEFT DELETING LEADING '0'.
          ls_result-SalesOrderNumber = lv_so.
        ENDIF.
      ...
      ls_result-Director = lv_ktoan.
      ls_result-HeadOfDepartment = lv_tp.
      ls_result-Cashier = lv_tkho.
      ls_result-Department = lv_bp.
