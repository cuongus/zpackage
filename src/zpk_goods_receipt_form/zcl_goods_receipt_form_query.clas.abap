CLASS zcl_goods_receipt_form_query DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS ZCL_GOODS_RECEIPT_FORM_QUERY IMPLEMENTATION.
    ...
    TRY.
        ...
        IF lo_filter IS BOUND.
          ...
          LOOP AT lt_filters INTO DATA(ls_filter).
            IF ls_filter-name = 'SHIPPER'.
              DATA(lr_giaohang) = lt_filters[ name = 'SHIPPER' ]-range.
              READ TABLE lr_giaohang INTO DATA(ls_range) INDEX 1.
              DATA(lv_giaohang) = ls_range-low.
            ...
            ELSEIF ls_filter-name = 'DEPARTMENT'.
              DATA(lr_qlkho) = lt_filters[ name = 'DEPARTMENT' ]-range.
              READ TABLE lr_qlkho INTO ls_range INDEX 1.
              DATA(lv_qlkho) = ls_range-low.
            ENDIF.
          ENDLOOP.
*          DATA(lr_giaohang) = lt_filters[ name = 'SHIPPER' ]-range.
*          DATA(lr_tkho) = lt_filters[ name = 'CASHIER' ]-range.
...
*        DATA(lv_ktoan) = ls_range-low.
    ...
    SELECT FROM i_materialdocumentitem_2 AS a
      ...
      FIELDS a~materialdocument,
             ...
             f~supplierfullname AS suppliername,
             g~plantname
...
*    ENDIF.
      ...
      LOOP AT GROUP <group> INTO DATA(ls_item).
        ...
        IF lv_no = 1.
          ...
          DATA(ls_result) = VALUE zc_goods_receipt_form(
             ...
             vendorname           = ls_item-suppliername
    ...
    LOOP AT lt_result INTO ls_result.
      ...
      ls_result-Shipper = lv_giaohang.
      ls_result-Director = lv_ktoan.
      ls_result-Department  = lv_qlkho.
      MODIFY lt_result FROM ls_result INDEX sy-tabix.
    ENDLOOP.
    " 6. Set response
