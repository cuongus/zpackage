CLASS zcl_get_fillter_thetaisan DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-DATA:
     "Instance Singleton
     mo_instance      TYPE REF TO zcl_get_fillter_thetaisan.

    CLASS-METHODS:
      "Contructor khỏi tạo đối tượng
      get_Instance RETURNING VALUE(ro_instance) TYPE REF TO zcl_get_fillter_thetaisan,
...
CLASS ZCL_GET_FILLTER_THETAISAN IMPLEMENTATION.
  ...
  METHOD get_fillter_app.
    ...
    LOOP AT lr_ranges INTO DATA(ls_ranges).
      CASE ls_ranges-name.
        ...
        WHEN 'MAINASSET'.
          MOVE-CORRESPONDING ls_ranges-range TO ir_mainasset.
        WHEN 'SUBASSET'.
          MOVE-CORRESPONDING ls_ranges-range TO ir_subasset.
        WHEN 'ASSETCLASS'.
          MOVE-CORRESPONDING ls_ranges-range TO ir_assetclass.
        WHEN 'COSTCENTER'.
          MOVE-CORRESPONDING ls_ranges-range TO ir_costcenter.
        WHEN 'DESCRIP'.
           MOVE-CORRESPONDING ls_ranges-range TO ir_desc.
        WHEN 'ADDDESCRIP'.
           MOVE-CORRESPONDING ls_ranges-range TO ir_add.
        WHEN OTHERS.
      ENDCASE.
    ENDLOOP.
  ENDMETHOD.
