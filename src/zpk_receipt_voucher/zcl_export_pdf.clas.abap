CLASS zcl_export_pdf DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*
*           e_phieu_thu  TYPE TABLE OF zc_receipt_voucher.


    TYPES:
      keys_pkt     TYPE TABLE FOR ACTION IMPORT zc_receipt_voucher~btnprintpdf,
      result_pkt   TYPE TABLE FOR ACTION RESULT zc_receipt_voucher~btnprintpdf,
      mapped_pkt   TYPE RESPONSE FOR MAPPED EARLY zc_receipt_voucher,
      failed_pkt   TYPE RESPONSE FOR FAILED EARLY zc_receipt_voucher,
      reported_pkt TYPE RESPONSE FOR REPORTED EARLY zc_receipt_voucher.
...
*                                ir_compaycode TYPE tt_ranges
  ...
  PRIVATE SECTION.
    CLASS-DATA:
      ir_companycode        TYPE zcl_receipt_voucher=>tt_ranges,
      ir_accountingdocument TYPE zcl_receipt_voucher=>tt_ranges,
      ir_fiscalyear         TYPE zcl_receipt_voucher=>tt_ranges,
      ir_glaccount          TYPE zcl_receipt_voucher=>tt_ranges,
      ir_documentitem       TYPE zcl_receipt_voucher=>tt_ranges,
      ir_postingdate        TYPE zcl_receipt_voucher=>tt_ranges,
      ir_documentdate       TYPE zcl_receipt_voucher=>tt_ranges,
      ir_documenttype       TYPE zcl_receipt_voucher=>tt_ranges,
      ir_customer           TYPE zcl_receipt_voucher=>tt_ranges,
...
CLASS ZCL_EXPORT_PDF IMPLEMENTATION.
...
*
*
    zcl_receipt_voucher=>get_phieu_thu_1(
      EXPORTING
        ...
        ir_name = ir_name
      IMPORTING
        e_phieu_thu         = DATA(lt_phieuketoan)
*        e_phieuketoan_items   = DATA(lt_phieuketoanitems)
    ).
...
*    READ TABLE lt_data INTO DATA(ls_data) WITH KEY accountingdocument = k-AccountingDocument
...
*      bannerxml = bannerxml &&  |<Row1>| &&
*                                    |<Doituong>{ ls_data-doituong }</Doituong>| &&
*                                |</Row1>| &&
*                                |<Row2>| &&
*                                    |<Diachi>Tài chính kế toán / Finance and Accounting</Diachi>| &&
*                                |</Row2>| &&
*                                |<Row3>| &&
*                                    |<Diengiai>{ ls_data-diengiai }</Diengiai>| &&
*                                |</Row3>| &&
*                                |<Row4>| &&
*                                    |<Sotien>{ ls_data-sotien }</Sotien>| &&
*                                |</Row4>| &&
*                                |<Row5>| &&
*                                    |<amountInWords>{ ls_data-sotien }</amountInWords>| &&
*                                |</Row5>| &&
*                                |<Row6>| &&
*                                    |<Attached >Chứng từ kèm theo</Attached >| &&
*                                |</Row6>|.
...
*  |<formFooter>| &&
*  |<amountInWords>{ ls_data-sotien }</amountInWords>| &&
*                 |<Foreign>{ ls_data-sotien }</Foreign>| &&
*                 |<ConvertedAmount>{ ls_data-sotien }</ConvertedAmount>| &&
*  |</formFooter>|.
...
*    DATA(o_gen_adobe) = NEW zcl_gen_adobe( ).
    ...
    lv_filename = |PhieuThu_{ ls_data-CompanyCode }{ ls_data-FiscalYear }{ ls_data-AccountingDocument }|.
  ...
  result = VALUE #( FOR key IN keys (
    ...
    DATA: ls_mapped LIKE LINE OF mapped-zc_receipt_voucher.
*    ls_mapped-%tky         = k-%tky.
    INSERT CORRESPONDING #( ls_mapped ) INTO TABLE mapped-zc_receipt_voucher.
