CLASS zcl_receipt_voucher DEFINITION
  PUBLIC
*  FINAL
  ...
  PUBLIC SECTION.
    TYPES: BEGIN OF ty_range_option,
           ...
           tt_phieu_thu TYPE TABLE OF zc_receipt_voucher.


    "Custom Entities
    INTERFACES if_rap_query_provider.

    CLASS-DATA: gt_phieu_thu TYPE tt_phieu_thu.

    CLASS-DATA: gv_generaldirector TYPE string,
                gv_cashier         TYPE string,
                gv_preparedby      TYPE string,
                gv_receiver        TYPE string,
                gv_chiefaccountant TYPE string.


    CLASS-METHODS: get_phieu_thu_1 IMPORTING ir_compaycode         TYPE tt_ranges
                                   ...
                                   EXPORTING e_phieu_thu           TYPE tt_phieu_thu
...
CLASS ZCL_RECEIPT_VOUCHER IMPLEMENTATION.
...
*      FOR ALL ENTRIES IN @lt_journalentry
      ...
      SELECT FROM i_operationalacctgdocitem
        FIELDS companycode, accountingdocument, fiscalyear, accountingdocumentitem,
          glaccount, operationalglaccount, isnegativeposting, debitcreditcode, supplier, customer,
...
*          FOR ALL ENTRIES IN @lt_journalentry
      ...
      SELECT FROM i_onetimeaccountcustomer AS a
        FIELDS *
*        FOR ALL ENTRIES IN @lt_journalentryitem
        WHERE companycode IN @ir_compaycode
          AND accountingdocument IN @ir_accountingdocument
          AND fiscalyear in @ir_fiscalyear
*          AND accountingdocumentitem IN @ir_
    ...
    LOOP AT lt_journalentry INTO DATA(ls_journalentry).
      ...
      IF lv_check = abap_true.
        ls_phieu_thu-companycode = ls_journalentry-companycode.
        ls_phieu_thu-postingdate = ls_journalentry-postingdate.
        ls_phieu_thu-accountingdocument = ls_journalentry-accountingdocument.
        ls_phieu_thu-fiscalyear = ls_journalentry-fiscalyear.
        ls_phieu_thu-accountingdocumenttype = ls_journalentry-accountingdocumenttype.
        ls_phieu_thu-transactioncurrency = ls_journalentry-transactioncurrency.
        ls_phieu_thu-creationuser = ls_journalentry-accountingdoccreatedbyuser.
        ls_phieu_thu-creationdate = ls_journalentry-accountingdocumentcreationdate.
        ls_phieu_thu-creationtime = ls_journalentry-creationtime.
          ...
          LOOP AT lt_bseg INTO ls_bseg FROM lv_index.
            ...
            IF ( ls_bseg-isnegativeposting IS INITIAL AND ls_bseg-debitcreditcode = 'S' ) OR
              ...
              append_unique_account(
                ...
                CHANGING cv_target_string = ls_phieu_thu-acc_s ).
              ls_phieu_thu-businesspartner = COND #( WHEN ls_phieu_thu-businesspartner IS INITIAL AND ls_bseg-customer IS NOT INITIAL
                                                  THEN ls_bseg-customer
                                                   ).
              ls_phieu_thu-diengiai = COND #( WHEN ls_phieu_thu-diengiai IS INITIAL AND ls_bseg-documentitemtext IS NOT INITIAL
                                             THEN ls_bseg-documentitemtext ).
              ls_phieu_thu-sotien = ls_phieu_thu-sotien + ls_bseg-absoluteamountincocodecrcy.
            ELSE.
              append_unique_account(
                EXPORTING iv_new_account   = lv_glaccount
                CHANGING cv_target_string = ls_phieu_thu-acc_h ).
              ls_phieu_thu-businesspartner = COND #( WHEN ls_phieu_thu-businesspartner IS INITIAL AND ls_bseg-customer IS NOT INITIAL
                                                  THEN ls_bseg-customer
                                                   ).
              ls_phieu_thu-diengiai = COND #( WHEN ls_phieu_thu-diengiai IS INITIAL AND ls_bseg-documentitemtext IS NOT INITIAL
            ...
            IF ls_journalentry-AccountingDocumentType = 'SK'.
              READ TABLE lt_companycode_info INTO DATA(ls_companycode_info)
                WITH KEY companycode = ls_journalentry-companycode.
              IF sy-subrc = 0.
                ls_phieu_thu-diachi = ls_companycode_info-companycodeaddr.
              ENDIF.

               ls_phieu_thu-doituong = ls_journalentryitem-yy1_addittext2_jei.
...
*              ENDIF.
            ELSE.
              CLEAR lv_bp.
              lv_bp = ls_phieu_thu-businesspartner.
              ...
              IF sy-subrc = 0 AND lv_bp+0(2) = '99'.
                ls_phieu_thu-doituong = ls_bsec_match-businesspartnername1.
                ls_phieu_thu-diachi = |{ ls_bsec_match-streetaddressname } { ls_bsec_match-cityname }|.
              ELSE.
                ...
                wa_document-customer = ls_phieu_thu-businesspartner.
                ...
                ls_phieu_thu-doituong = ls_supplier_detail-bpname.
                ls_phieu_thu-diachi = ls_supplier_detail-bpaddress.
              ENDIF.
            ENDIF.
          ENDLOOP.
        ENDIF.
        IF ls_journalentryitem-documentitemtext IS NOT INITIAL.
          ls_phieu_thu-diengiai = ls_journalentryitem-documentitemtext.
        ENDIF.

        READ TABLE ir_generaldirector INTO DATA(ls_ge) INDEX 1.
        IF sy-subrc = 0 .
          ls_phieu_thu-generaldirector = ls_ge-low.
        ENDIF.

        READ TABLE ir_PreparedBy INTO DATA(ls_pe) INDEX 1.
        IF sy-subrc = 0 .
          ls_phieu_thu-PreparedBy = ls_pe-low.
        ENDIF.

        READ TABLE ir_cashier INTO DATA(ls_ca) INDEX 1.
        IF sy-subrc = 0 .
          ls_phieu_thu-Cashier = ls_ca-low.
        ENDIF.

        READ TABLE ir_chiefaccountant INTO DATA(ls_ch) INDEX 1.
        IF sy-subrc = 0 .
          ls_phieu_thu-ChiefAccountant = ls_ch-low.
        ENDIF.

        READ TABLE ir_receiver INTO DATA(ls_re) INDEX 1.
        IF sy-subrc = 0 .
          ls_phieu_thu-Receiver = ls_re-low.
        ENDIF.

        READ TABLE ir_name INTO DATA(ls_name) INDEX 1.
        IF sy-subrc = 0 and ls_name-low is not INITIAL .
          ls_phieu_thu-Doituong = ls_name-low.
          ls_phieu_thu-name = ls_name-low.
        ENDIF.

        APPEND ls_phieu_thu TO e_phieu_thu.
        CLEAR: ls_phieu_thu, lv_glaccount.
...
**--- Custom Entities ---**
    ...
    DATA: lo_report_fi TYPE REF TO zcl_receipt_voucher.
...
*    ENDIF.
    ...
    DATA: lt_phieu_thu TYPE tt_phieu_thu.

    LOOP AT gt_phieu_thu INTO DATA(ls_phieu_thu).
      IF sy-tabix > ls_page_info-offset.
        IF sy-tabix > max_rows.
          EXIT.
        ELSE.
          APPEND ls_phieu_thu TO lt_phieu_thu.
        ENDIF.
      ENDIF.
    ENDLOOP.

    IF io_request->is_total_numb_of_rec_requested( ).
      io_response->set_total_number_of_records( lines( gt_phieu_thu ) ).
    ENDIF.

    IF io_request->is_data_requested( ).
      io_response->set_data( lt_phieu_thu ).
    ENDIF.
