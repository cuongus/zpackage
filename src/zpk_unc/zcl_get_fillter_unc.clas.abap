CLASS zcl_get_fillter_unc DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-DATA:
     "Instance Singleton
     mo_instance      TYPE REF TO zcl_get_fillter_unc.

    CLASS-METHODS:
      "Contructor khỏi tạo đối tượng
      get_Instance RETURNING VALUE(ro_instance) TYPE REF TO zcl_get_fillter_unc,
...
CLASS ZCL_GET_FILLTER_UNC IMPLEMENTATION.
  ...
  METHOD get_fillter_app.
    ...
    LOOP AT lr_ranges INTO DATA(ls_ranges).
      CASE ls_ranges-name.
        ...
        WHEN 'RUNDATE'.
          MOVE-CORRESPONDING ls_ranges-range TO ir_rundate.
        WHEN 'IDENTIFICATION'.
          MOVE-CORRESPONDING ls_ranges-range TO ir_identification.
        WHEN 'PRINTDATE'.
